...
void ISkillCast::CastSkill(const FSkillRowHandle& InSkill, const FVector& InTargetLocation, AActor* InTargetActor)
{
	...
	check(Self);
	Self->GetWorld()->GetGameInstance()->GetSubsystem<USkillSubsystem>()->CastSkill
	(
		InSkill,
		CommonPawn,
		InTargetLocation,
